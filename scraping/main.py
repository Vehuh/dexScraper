...
if __name__ == "__main__":
    ...
    while True:
        ...
        # TODO: create thread for each token in token_list
        # data for each token needs to be get every 1-2 minutes
        # we need to have a way, to stop the threads, based on "updated" token_list from the watch_list result
        for i in range(len(token_list)):
            ...
            network = "solana"
            if (
                (network in scraper.network_ids_for_gecko_terminal)
                and (token_address is not None)
                and (token_address != "string")
            ):  # If valid name of network

                try:
                    pool_name, pool_address, price = scraper.get_top_pool_from_gecko(
